...
defmodule PostHTMLFormatColocatedSigilFormatter do
  ...
  def format(contents, opts) do
    cond do
      opts[:modifiers] == ~c'css' and
          contents |> String.trim() |> String.starts_with?("<style TEMPORARY>") ->
        contents
        |> String.trim()
        |> String.trim_leading("<style TEMPORARY>")
        |> String.trim_trailing("</style>")
        |> String.trim_leading("\n")

      opts[:modifiers] == ~c'js' and
          contents |> String.trim() |> String.starts_with?("<script TEMPORARY>") ->
        contents
        |> String.trim()
        |> String.trim_leading("<script TEMPORARY>")
